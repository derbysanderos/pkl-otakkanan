...
export default function LocationPage() {
  ...
  const handleDeleteLocation = (location) => {
    if (window.confirm(`Are you sure you want to delete ${location.office_name}?`)) {
    ...
    <main className="h-full overflow-y-auto bg-gray-50">
      <div className="container grid px-6 mx-auto">
        <div className="my-6 flex justify-between items-center">
          <h2 className="text-2xl font-semibold text-gray-700">Tabel Location</h2>
          <button
          ...
          <div className="w-full overflow-x-auto bg-white p-4">
            <div className="dt-container dt-empty-footer">
                    ...
                    <tbody className="divide-y text-xs text-gray-600">
                      {paginatedLocations.map((l, index) => (
                        <tr key={index} className="h-10">
                          <td className="text-center py-[8px] px-[10px]">{index + 1}</td>
                          <td className="text-center py-[8px] px-[10px]">{l.office_name}</td>
                          <td className="text-center py-[8px] px-[10px]">{l.latitude}</td>
                          <td className="text-center py-[8px] px-[10px]">{l.longtitude}</td>
                          <td className="text-center py-[8px] px-[10px]">
                            <div className="btn-group flex gap-6 justify-center">
                              ...
                              <button
                                onClick={() => handleShowMap(l.latitude, l.longtitude)}
              ...
              <div className="flex justify-between mx-10 my-5">
                <div>
                  <p>
                    Showing {paginatedLocations.length} of {filter.length} entries
                  </p>
                </div>
